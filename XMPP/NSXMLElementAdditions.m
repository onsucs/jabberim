...
- (void)removeElementsForName:(NSString *)name
{
	...
	{
		if ([[[self childAtIndex:i] name] isEqualToString:name])
		{
...
/**
 * Adds a unique element, removing the old one if necessary
 **/

- (void)setStringValue:(NSString *)string forElementWithName:(NSString *)name
{
	[self removeElementsForName:name];
	[self addChild:[NSXMLElement elementWithName:name stringValue:string]];
}

/**
 * Adds a unique element, removing the old one if necessary
 * Settng to nil removes the element
 **/
