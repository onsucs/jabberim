...
// Define XMLString output formats
#define XMLSTRING_STREAM_OPTONS NSXMLNodeCompactEmptyElement
#define XMLSTRING_DEBUG_OPTIONS NSXMLNodePrettyPrint|NSXMLNodeCompactEmptyElement
...
enum XMPPStreamFlags
{
	...
	kValidatesCertificateChain    = 1 << 4,  // If set, the certificate chain will be validated (defaults YES)
...
- (NSString *)streamXMLForElement:(NSXMLElement *)element;
@end
...
- (id)initWithDelegate:(id)aDelegate
{
	if((self = [super init]))
	{
		...
		flags = kValidatesCertificateChain;
...
/**
 * Sometimes you just can't validate this certificate chain at all.
 * You really should not turn this off in production code, but in some test cases it may be needed
 **/
- (BOOL)validatesCertificateChain
{
	return (flags & kValidatesCertificateChain);
}
...
- (void)setValidatesCertificateChain:(BOOL)flag
{
	if (flag)
	{
		flag |= kValidatesCertificateChain;
	}
	else
	{
		flags &= ~kValidatesCertificateChain;
		NSLog(@"WARNING: XMPPStream: Turning off certificate chain validation. This is dangerous.");
	}
}
...
- (void)registerUser:(NSString *)username withPassword:(NSString *)password
{
	...
	if(state == STATE_CONNECTED)
	{
		if([self supportsInBandRegistration])
		{
			...
			[asyncSocket writeData:[[self streamXMLForElement:iqElement] dataUsingEncoding:NSUTF8StringEncoding]
				...
				resource:(NSString *)resource
{
	...
	if(state == STATE_CONNECTED)
	{
		...
		else if([self supportsPlainAuthentication])
		{
			...
			[asyncSocket writeData:[[self streamXMLForElement:auth] dataUsingEncoding:NSUTF8StringEncoding]
					   withTimeout:TIMEOUT_WRITE
							   tag:TAG_WRITE_STREAM];
...
- (void)sendElement:(NSXMLElement *)element
{
	if(state == STATE_CONNECTED)
	{
		NSString *elementStr = [self streamXMLForElement:element];
...
- (void)sendOpeningNegotiation
{
	...
	{
		NSString * const temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0' to='%@'>";
		s2 = [NSString stringWithFormat:temp, xmlns, xmlns_stream, xmppHostName];
	}
	else
	{
		NSString * const temp = @"<stream:stream xmlns='%@' xmlns:stream='%@' version='1.0'>";
...
- (void)startTLS
{
	...
	// Turn off certificate chain validation. This is very dangerous.
...
- (void)handleStreamFeatures
{
	...
	if(f_bind)
	{
		...
		if([authResource length] > 0)
		{
			...
			[asyncSocket writeData:[[self streamXMLForElement:iq] dataUsingEncoding:NSUTF8StringEncoding]
					   withTimeout:TIMEOUT_WRITE
							   tag:TAG_WRITE_STREAM];
		}
		else
...
- (void)handleAuth1:(NSXMLElement *)response
{
	if([self supportsDigestMD5Authentication])
	{
		...
		else
		{
			...
			[asyncSocket writeData:[[self streamXMLForElement:cr] dataUsingEncoding:NSUTF8StringEncoding]
					   withTimeout:TIMEOUT_WRITE
							   tag:TAG_WRITE_STREAM];
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	...
	else if([[element name] isEqualToString:@"iq"])
	{
		...
		{
			[delegate xmppStream:self didReceiveIQ:[[[XMPPIQStanza alloc] initWithXMLElement:element] autorelease]];
		}
		else if(DEBUG_DELEGATE)
		{
	...
	else if([[element name] isEqualToString:@"message"])
	{
		...
		{
			[delegate xmppStream:self didReceiveMessage:[[[XMPPMessageStanza alloc] initWithXMLElement:element] autorelease]];
		}
		else if(DEBUG_DELEGATE)
		{
	...
	else if([[element name] isEqualToString:@"presence"])
	{
		...
		{
			[delegate xmppStream:self didReceivePresence:[[[XMPPPresenceStanza alloc] initWithXMLElement:element] autorelease]];
		}
		else if(DEBUG_DELEGATE)
		{
...
- (NSString *)streamXMLForElement:(NSXMLElement *)element
{
	DDLogSend(@"SEND: %@", [element XMLStringWithOptions:XMLSTRING_DEBUG_OPTIONS]);
	return [element XMLStringWithOptions:XMLSTRING_STREAM_OPTONS];
}	
