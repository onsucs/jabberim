...
- (void)sendPingToResource:(XMPPResource *)resource
{
	...
	NSXMLElement *iq = [NSXMLElement elementWithName:@"iq"];
	[iq addAttributeWithName:@"to" stringValue:[[resource jid] fullString]];
	[iq addAttributeWithName:@"type" stringValue:@"get"];
	[iq addAttributeWithName:@"id" stringValue:pingID];
	[iq addChild:ping];
...
- (void)xmppClient:(XMPPClient *)sender didReceiveIQ:(XMPPIQStanza *)iq
{
	...
	if([type isEqualToString:@"result"] || [type isEqualToString:@"error"])
	{
		...
		NSString *pingID = [iq uniqueIdentifier];
	...
	else if([type isEqualToString:@"get"])
	{
		...
		if(ping)
		{
			...
			[pong addAttributeWithName:@"to" stringValue:[[iq fromJID] fullString]];
			[pong addAttributeWithName:@"type" stringValue:@"result"];
			[pong addAttributeWithName:@"id" stringValue:[iq uniqueIdentifier]];
