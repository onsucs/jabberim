				...
				error:(NSError **)errPtr
{
	if(theDelegate == NULL)
	{
		NSString * const message = @"Attempting to connect without a delegate. Set a delegate first.";
		[NSException raise:AsyncSocketException format:message];
	}

	if(theSocket4 != NULL || theSocket6 != NULL)
	{
		NSString * const message = @"Attempting to connect while connected or accepting connections. Disconnect first.";
		[NSException raise:AsyncSocketException format:message];
	}
...
- (CFIndex)readIntoBuffer:(UInt8 *)buffer maxLength:(CFIndex)length
{
	...
	{
		// DEBUG HACK
		CFIndex bytesRead = CFReadStreamRead(theReadStream, buffer, length);
//		NSLog(@"READ:%@", [[[NSString alloc] initWithData:[NSData dataWithBytes:buffer length:length] encoding:NSUTF8StringEncoding] autorelease]);
		return bytesRead;
	}
}
